...
file_handler = logging.FileHandler(filename=os.path.join(pa.logs_folder, 'train_orient-{}.log'.format(datetime.now().strftime('%Y_%m_%d_%H_%M'))))
...
def build_training_ops(loss_tensor, itr_per_epoch):
    ...
    decaying_learning_rate = tf.train.exponential_decay(
        LEARNING_RATE, global_step, itr_per_epoch*10, 0.1, staircase=True)
...
def main(argv=None):
    ...
    with tf.device('/GPU:0'):
        with tf.variable_scope("cls_net"):
            ...
            if len(feats) == 2:
                ...
                if 'cooc_map' in feats:
                    ...
                    pred = rnn_network.build_network_one_feat([tf_btf_2], pa.lstm_units, num_classes,
                                                              name='jc_net')
        ...
        for epoch in range(EPOCHS):
            ...
            for idx in tqdm(range(itr_per_epoch_train)):
                ...
                if len(feats) == 2:
                    ...
                    feed_dict = {tf_btl: btl, tf_btf_1: btf_1, tf_btf_2: btf_2}
                elif len(feats) == 1:
                    if 'jc' in feats:
                        if pa.rnn_saved_jcla_folder != "":
                            btf_2 = video_utils.extract_jcla(train_idxes_filled, idx, batch_size, use_folder='train')
                            ...
                            btf_2 = video_utils.extract_less_bone_length_joint_angle_sign(btjc)
                        feed_dict = {tf_btl: btl, tf_btf_2: btf_2}
                    elif 'cooc_map' in feats:
                        btf_1 = video_utils.extract_cooc_map(train_idxes_filled, idx, batch_size, use_folder='train')
                        feed_dict = {tf_btl: btl, tf_btf_1: btf_1}
            ...
            rnn_saver.save(sess, pa.checkpoint_path_cls + "/ckpt", global_step=epoch)
            ...
            for val_idx in tqdm(range(itr_per_epoch_val)):
                if pa.rnn_saved_jcla_folder != "":
                    ...
                    btjc_test, btl_test = video_utils.extract_jc_labels(val_idxes, val_idx, batch_size, labels, use_folder='val')
                if len(feats) == 2:
                    ...
                    if 'jc' in feats:
                        if pa.rnn_saved_jcla_folder != "":
                            ...
                            btf_2_test = video_utils.extract_less_bone_length_joint_angle_sign(btjc_test)
                        feed_dict_test = {tf_btl: btl_test, tf_btf_2: btf_2_test}
                    elif 'cooc_map' in feats:
                        btf_1_test = video_utils.extract_cooc_map(val_idxes, val_idx, batch_size, use_folder='val')
                        feed_dict_test = {tf_btl: btl_test, tf_btf_1: btf_1_test}
            ...
            if val_acc > best["acc"]:
                ...
                bestsaver.save(sess, pa.checkpoint_path_cls+"/best/ckpt", global_step=epoch)
